...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x: tf.Tensor, training: bool, mask: Optional[tf.Tensor]) -> tf.Tensor:
        ...
        attn_output, _ = self.multi_head_attention.call(x, x, x, mask)  # (batch_size, input_seq_len, d_model)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # self_attention for the target sequence. look_ahead_mask prevents from using "future" tokens.
        # key = value = query matrices. attn1 shape is (b, target_seq_len, d_model)
        attn1, attn_weights_block1 = self.multi_head_attention_1.call(x, x, x, look_ahead_mask)
        attn1 = self.dropout1(attn1, training=training)
        out1 = self.layernorm1(attn1 + x)

        # Attention between input and target sequences. key and value are the same matrices.
        # padding_mask prevents the output sequence from attending to a lot of padded values in the input sequence.
        attn2, attn_weights_block2 = self.multi_head_attention_2.call(
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self,
        ...
        for i in range(self.num_layers):
            x, block1, block2 = self.decoder_layers[i].call(x, enc_output, training, look_ahead_mask, padding_mask)
...
class Transformer(tf.keras.Model):
    def __init__(self,
        ...
        self.encoder = Encoder(
            num_layers, d_model, num_heads, dff, input_vocab_size, max_posit_encode_input, dropout_rate
        )
        self.decoder = Decoder(
            num_layers, d_model, num_heads, dff, target_vocab_size, max_posit_encode_target, dropout_rate
        )
